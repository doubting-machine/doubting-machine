...
def handle_sigint(signum, frame):
    ...
    sys.stderr.write("doubting-machine: SIGINT received, exiting.\n")
    sys.stderr.flush()
    sys.exit(0)
...
def run(provider_api_key, github_token, mode):
    ...
    doubting_machine = {settings['name']}
    try:
        text, thoughts = machine(raw_input)
        # json.dump((thoughts_output, text_output), sys.stdout)
        sys.stdout.write(f'{doubting_machine}: (to itself) {thoughts}\n')
        sys.stdout.write(f'{doubting_machine}: {text}\n')
        sys.stdout.flush()
    except Exception as e:
        sys.stderr.write(f'{doubting_machine} did not work {e}')
        sys.stderr.flush()
        sys.exit(0)
